...
export default function PassOption() {
  ...
  return (
      ...
      onSubmit={async (e) => {
        ...
        try {
          ...
          await fetch('/api/sync/orders', {
            ...
            body: JSON.stringify({
              orders: [
                { name, menu: null, status: 'passed', brandKey: state.currentBrand ?? null },
              ],
              date: today,
            }),
          });
    ...
    >
      <h3 className="font-semibold">주문안함</h3>
      {isClosed && (
      ...
      <input
        ...
        disabled={isClosed}
      />
      <button className="sbx-btn-secondary w-full disabled:cursor-not-allowed disabled:opacity-50" disabled={isClosed}>
        {isClosed ? '주문 마감' : '패스'}
      </button>
    </form>
  );
